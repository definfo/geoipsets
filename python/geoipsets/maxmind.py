...
class MaxMindProvider(utils.AbstractProvider):
    ...
    def generate(self):
        ...
        with ZipFile(Path(zip_file.name), "r") as zip_ref:
            ...
            if self.ipv4:
                self.build_sets(
                    id_cc_map, zip_ref, zip_dir_prefix, utils.AddressFamily.IPV4
                )

            if self.ipv6:
                self.build_sets(
                    id_cc_map, zip_ref, zip_dir_prefix, utils.AddressFamily.IPV6
                )
    ...
    def check_checksum(self, zip_ref):
        ...
        if expected_sha256sum != computed_sha256sum:
            raise SystemExit(
                "ERROR: Computed zip file digest '{0}' does not match expected value '{1}'".format(

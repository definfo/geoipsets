...
def get_config(cli_args=None):
    ...
    parser.add_argument(
        ...
        choices={
            ...
            utils.Firewall.IP_TABLES.value,
            utils.Firewall.FIREWALLD.value,
        },
        help="firewall(s) to build sets for (default: {0})".format(
            utils.Firewall.NF_TABLES.value
        ),
    )
    parser.add_argument(
        ...
        help="IP protocol(s) to build sets for (default: {0})".format(
            utils.AddressFamily.IPV4.value
        ),
    )
    parser.add_argument(
        "-l",
    ...
    parser.add_argument(
        ...
        help="directory where geoipsets should be saved (default: {0})".format(
            default_output_dir
        ),
    )
    parser.add_argument(
    ...
    if (address_family := parser.parse_args(cli_args).address_family) is not None:
        ...
        if (
            valid_conf_file
            and (address_family := general.get("address-family")) is not None
        ):
    ...
    if (country_arg := parser.parse_args(cli_args).countries) is not None:
        ...
        try:
            ...
            print(
                "file '{0}' does not exist, parsing as list instead".format(country_arg)
            )
...
def main():
    opts = get_config()
    providers: Literal["iptables", "nftables", "firewalld"] | None = opts.get(
        "provider"
    )
    assert providers is not None, (
        "ERROR: provider is not set, should be one of iptables, nftables, or firewalld"
    )
    print("Building geoipsets...")

    if "maxmind" in providers:
        mmp = maxmind.MaxMindProvider(
            opts.get("firewall"),

...
def test_runas_module_version():
    ...
    """
    out = subprocess.run(
        ["python", "-m", "geoipsets", "--version"], capture_output=True, text=True
    )
...
@pytest.mark.parametrize(
    "option, value",
    [("provider", "DbIp"), ("firewall", "NfTables"), ("address-family", "IpV4")],
)
...
@pytest.mark.parametrize("provider", ["maxmind", "dbip"])
